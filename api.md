...
| 方法 | 作用                                       | 参数                                                         | 返回值                         |
| ---- | ------------------------------------------ | ------------------------------------------------------------ | ------------------------------ |
| post | 发送消息                                   | from，uint，用户id  （不知道能不能从cookie读，后面可能不需要） | status,bool，结果是否正常      |
|      |                                            | to,  uint，对象id                                            | msg,string,结果信息            |
|      |                                            | msg,  string，信息主体，目前设定一次信息不超过140个字        |                                |
| get  | 获取消息变更，所有未读信息，发送的信息被读 | history,bool,是否为查看历史信息，（为假）                    | status，bool，结果是否成功     |
|      |                                            |                                                              | msg，string，结果信息          |
|      |                                            |                                                              | unread,json array,未读信息数组 |
|      |                                            |                                                              | read,json array,已读信息数组   |
| get  | 获取消息，个体间历史消息                   | history,bool,是否为查看历史信息，（为真继续                  | status,bool，结果是否正常      |
|      |                                            | from，uint，（系统为0                                        | msg,string,结果信息            |
|      |                                            | to,  uint，对象id，用户id  （不知道能不能从cookie读，后面可能不需要） | 信息数组                       |
| post | 撤回单条信息，已读的不能撤回               | mid，信息id                                                  | status,bool，结果是否正常      |
|      |                                            |                                                              | msg,string,结果信息            |
